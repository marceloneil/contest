...
PII calc(int teacher, int ttime){
  ...
  for(int i = 0; h[teacher] - ((i) * e[teacher]) > 0 && (i+1) * p[teacher] <= ttime; i++){ // the number of times the teacher appears
    ...
    PII otherTeachers = calc(teacher + 1, ttime - ((i+1) * p[teacher]));
    happiness += otherTeachers.F;

    if(happiness > ansMaxHappiness){
      ansMaxHappiness = happiness;
      ansMinTime = otherTeachers.S + i+1;
    }else if(happiness == ansMaxHappiness){
      ansMinTime = min(ansMinTime, otherTeachers.S + i + 1);
    }
  }
